...
//Choisir une image de manière aléatoire
let randomeImage = images [Math.floor(Math.random() * images.length)];

pageContent.style.backgroundImage = `url(./images/${randomeImage.filename})`;
console.log(randomeImage);

    // Si pictInfo existe
    if (pictInfo){
        // vider la variable pictInfo
   
    while(pictInfo.firstChild){
        pictInfo.removeChild(pictInfo.firstChild);
    }
    
    // Ajouter du texte a la variable pictInfo
    
    let pictTitle = document.createElement('h1');
    let pictTitleContent = document.createTextNode(randomeImage.name);
    pictTitle.appendChild(pictTitleContent);
    pictInfo.appendChild(pictTitle);
    
    let pictAuthor = document.createElement('p');
    let pictAuthorContent = document.createTextNode(randomeImage.author);
    let pictData = document.createElement('p');
    let pictDataContent = document.createTextNode(randomeImage.exposure);
